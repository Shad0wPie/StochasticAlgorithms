...
%distribution on the interval (interval = [-x, +x]). For each layer an
%additional set of weights is introduced that correspond to the biases
%
%layerDimensions: list of the number of neurons in each layer (input first)    
    ...
    weights{1} = zeros(nFirstLayer, nInputs+1);
    for i=1:nFirstLayer
        for j=1:nInputs+1
    ...
    if nSecondLayer > 0
        weights{2} = zeros(nSecondLayer, nFirstLayer+1);
        for i=1:nSecondLayer
            for j=1:nFirstLayer+1
